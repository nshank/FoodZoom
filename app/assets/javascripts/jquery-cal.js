  ...
  calendar.changeDate = function(date) {
    ...
    var fillMonths = function(date) {
      ...
      for (var i = -options.months; i <= options.months; i++) {
        ...
        var span = $("<h5>").addClass("calMonthElement").attr("millis", d.getTime()).html(options.monthNames[d.getMonth()]).css("width",w);
    ...
    var fillDays = function(date) {
      ...
      for (var i = -options.days; i <= options.days; i++) {
        ...
        if (i == -options.days && options.showDayArrows) {
          ...
          span.html("<div class=day-name>" + options.dayNames[d.getDay()]+ "</div>" + "<span class=dayNumber>" + d.getDate() + "</span>");
					//.css("width",w);
